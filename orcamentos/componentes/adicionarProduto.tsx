...
export default function AdicionarProduto() {
  ...
  const adicionarProduto = (event: React.FormEvent<HTMLButtonElement>) => {
    ...
    if (!orcamentoSelecionada.itens) {
      ...
      selecionarOrcamento({ ...orcamentoSelecionada, itens: [{ ...item, id: 'Item1' }] });
  ...
  const removerProduto = (id: string) => {
    const novosItems = orcamentoSelecionada.itens.filter((iten) => iten.id !== id);
  ...
  return (
    <div>
      <table className="produto-table">
        ...
        <tbody id="produtos">
          {orcamentoSelecionada?.itens && orcamentoSelecionada.itens.map((item) => (
            <tr key={item.id}>
              <td>
                <input
                  type="text"
              ...
              <td>
                <span className="form-control bg-secondary text-white border-secondary m-1 w-100 ">{item.total}</span>
              </td>
              <td>
